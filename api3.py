...
def get_data(endpoint, filename, paginated=True, limit=100, max_pages=50):
    ...
    try:
        for page in tqdm(range(max_pages), desc=f"Fetching {endpoint}"):
...
def get_account_details(accounts):
    details = []
    for account in tqdm(accounts, desc="Fetching account details"):
        acc_id = account.get("id")
        if not acc_id:
            continue
        url = f"{BASE_URL}/Accounts/{acc_id}"
        try:
            res = requests.get(url, headers=HEADERS, verify=VERIFY_SSL)
            if res.status_code == 200:
                details.append(res.json())
            else:
                print(f"⚠️ Erreur pour compte {acc_id} : {res.status_code}")
        except Exception as e:
            print(f"❌ Exception sur compte {acc_id} : {e}")
        time.sleep(0.2)
    save_to_file("accounts_details.json", details)

def get_safes_members(safes):
    members = {}
    for safe in tqdm(safes, desc="Fetching safe members"):
        safe_id = safe.get("safeUrlId")
        if not safe_id:
            continue
        url = f"{BASE_URL}/Safes/{safe_id}/members"
        try:
            res = requests.get(url, headers=HEADERS, verify=VERIFY_SSL)
            if res.status_code == 200:
                members[safe["safeName"]] = res.json()
            else:
                print(f"⚠️ Erreur pour Safe {safe_id} : {res.status_code}")
        except Exception as e:
            print(f"❌ Exception sur Safe {safe_id} : {e}")
        time.sleep(0.2)
    save_to_file("safes_members.json", members)

def main():
    try:
        authenticate()

        accounts = get_data("/Accounts", "accounts.json")
        safes = get_data("/Safes", "safes.json")
        platforms = get_data("/Platforms", "platforms.json")
        users = get_data("/Users", "users.json")
